  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 right-4 z-10">
        <div className={`flex items-center gap-2 px-3 py-2 rounded-full text-xs font-semibold backdrop-blur-md border shadow-lg ${statusBadge.color} transform transition-all duration-300 group-hover:scale-105`}>
          {statusBadge.icon}
        ...
        <AnimatePresence>
          {isMenuOpen && (
            <motion.div
              ...
              className="absolute left-0 top-full mt-2 w-40 bg-white dark:bg-gray-900 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 py-2 z-10"
            >
              <button
                onClick={() => { onEdit(branch); setIsMenuOpen(false); }}
                className="w-full flex items-center gap-2 px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
              >
      ...
      <div className="flex flex-col items-center justify-center flex-1 pt-12 pb-4 px-6">
        ...
        <div className="flex items-center gap-2 mt-2">
          ...
          <label className="relative inline-flex items-center cursor-pointer">
            ...
            <div className={`absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform duration-200 ${branch.isTemporarilyClosed ? 'translate-x-5' : ''} ${isToggling ? 'animate-pulse' : ''}`}></div>
          </label>
          {branch.isTemporarilyClosed && (
            <span className="text-xs text-amber-600 dark:text-amber-400 font-medium flex items-center gap-1 ml-1"><EyeOff className="w-3 h-3" /> Gizli</span>
          )}
        </div>
      </div>
      {/* Alt bilgi: Aktiflik */}
      <div className="flex items-center justify-center pb-4 flex-col gap-1">
        <div className={`rounded-full px-3 py-1 text-xs font-semibold ${activeBadge}`}>{branch.branchStatus ? 'Aktif' : 'Pasif'}</div>
        ...
        {branch.BranchIsOpen !== undefined && (
          <div className="mt-1 text-xs text-blue-600 dark:text-blue-300 font-semibold">
            API BranchIsOpen: {String(branch.BranchIsOpen)}
          </div>
        )}
      </div>
    </motion.div>
